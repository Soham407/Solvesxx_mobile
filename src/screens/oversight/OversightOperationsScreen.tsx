...
export function OversightOperationsScreen(_props: OversightOperationsScreenProps) {
  ...
  const attendanceExceptions = useMemo(
    () =>
      attendanceLog.filter(
        (entry) => entry.status === 'late' || entry.status === 'absent' || entry.geoStatus !== 'verified',
      ).length,
    [attendanceLog],
  );
  ...
  return (
    <ScreenShell
      ...
      description="Review checklist completion, gate throughput, and attendance exceptions from a single mobile operations tab."
    >
      <InfoCard>
        <View style={styles.headerRow}>
          <View style={styles.copyWrap}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Operations heartbeat</Text>
          ...
          return (
            <View key={guard.id} style={styles.recordCard} testID={`qa_oversight_guard_row_${index}`}>
              <View style={styles.headerRow}>
                ...
                <StatusChip label={guard.status.replace(/_/g, ' ')} tone={getGuardTone(guard.status)} />
              </View>
              <Text style={[styles.caption, { color: colors.foreground }]}>
                {guard.checklistCompleted}/{guard.checklistTotal} tasks done | Last seen {formatValue(guard.lastSeenAt)}
              </Text>
      ...
      <InfoCard>
        ...
        {visitorStats.length ? visitorStats.map((gate) => (
          <View key={gate.id} style={styles.recordCard}>
            <View style={styles.headerRow}>
              <View style={styles.copyWrap}>
                <Text style={[styles.recordTitle, { color: colors.foreground }]}>{gate.gateName}</Text>
                <Text style={[styles.caption, { color: colors.mutedForeground }]}>
        ...
        {attendanceLog.length ? attendanceLog.map((entry) => (
          <View key={entry.id} style={styles.recordCard}>
            <View style={styles.headerRow}>
              ...
              <StatusChip label={entry.status.replace(/_/g, ' ')} tone={getAttendanceTone(entry.status)} />
            </View>
